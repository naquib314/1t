...
export function formatTrace(
  ...
  highlightedPoints: Array<number> = null
) {
  const traces: any = volcanoData.map((volcanoDataTrace) => {
    ...
    if (highlightedPoints) {
      const symbols = Array(plotlyTrace.x.length)
        .fill(undefined)
        .map((_) => {
          return "circle";
        });
      const lineWidths = Array<number>(plotlyTrace.x.length).fill(undefined);
...
export const formatLayout = (
  ...
  annotations: Array<Partial<Plotly.Annotations>> = null
) => {
