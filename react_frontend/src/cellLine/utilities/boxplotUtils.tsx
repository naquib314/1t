...
export default class StackedBoxplotUtils {
  ...
  readonly margin = { top: 0, right: 40, bottom: 50, left: 80 };
  readonly svgWidth = 375; // Note: this needs to match the value set in CSS to prevent auto scaling
  ...
  initAxes(dataMatrix: Array<Array<number | null>>) {
    // calculate axis ranges
    const flattenedDataList = [].concat(...dataMatrix);
  ...
  labelXAxis(xAxisLabel: string) {
    ...
    this.svg
      ...
      .attr("x", (this.plotWidth + this.margin.left) / 2)
  ...
  markOutliers(labels: Array<string>, outliers: Array<Array<number>>) {
    ...
    labels.forEach((label, i) => {
      ...
      this.svg
        .selectAll(".pref-dep-outlier-point")
        .data(outlierCoordinates)
        .enter()
        .append("rect")
        .attr("class", ".pref-dep-outlier-point")
        .attr("x", (data) => data)
        .attr(
          "y",
  ...
  private static trimLabel(label: string, threshold: number): string {
    // add ellipses to labels by editing the text since svg doesn't support the text-overflow css class
    if (typeof label !== "string" || label.length <= threshold) return label;
    return label.substring(0, threshold - 2).concat("...");
  }
}
