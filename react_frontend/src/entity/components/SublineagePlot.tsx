...
class SublineagePlot extends React.Component<Props> {
  ...
  buildPlot = () => {
    ...
    if (this.handlePointClick) {
      ...
      this.plotElement.on("plotly_hover", (data: any) => {
        const dragLayer: HTMLElement = this.plotElement.querySelector(
    ...
    rawTraces.forEach((rawTrace) => {
      const label =
        rawTrace.lineage_level == 1
          ...
          : `${rawTrace.category} (${rawTrace.num_lines})`;
      ...
      numPlotsPerSubPlot.set(
        subPlotNumber,
        numPlotsPerSubPlot.get(subPlotNumber) + 1
      );
  ...
  buildLayout(
    ...
    for (let i = 0; i < numSubPlots; i++) {
      const yEnd = yCounter + numPlotsPerSubPlot.get(i + 1) * yIncrement;
      const domain = [1 - yEnd, 1 - yCounter];

      // @ts-expect-error Not sure how to express the dynamic string
...
export default hot(SublineagePlot);
