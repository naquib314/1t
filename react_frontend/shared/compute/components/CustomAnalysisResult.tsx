...
import {
  VolcanoPlot,
  VolcanoTrace,
} from "shared/interactive/components/VolcanoPlot";
...
import "shared/compute/styles/CustomAnalysisResult.scss";
...
> {
  ...
  renderResultTable = (slicedData: any) => {
    ...
    const columnsProps = visibleColsInData.map((x) => {
      // width specification is needed to make cols narrower
      return { key: x, width: 70 };
    });
    ...
    return (
      ...
      >
        <div key="resultVolcano" style={{ flex: "auto", height: "300px" }}>
          <VolcanoPlot Plotly={this.props.Plotly} {...plotProps} />
        </div>
      </div>
    );
  };
