...
import { encodeUrl } from "shared/common/utilities/route";
import {
...
import CustomAnalysisModal from "shared/compute/components/CustomAnalysisModal";
import { CustomAnalysisResult } from "shared/compute/components/CustomAnalysisResult";
import { Dataset } from "shared/compute/components/DatasetSelect";
import {
  AnalysisType,
  ComputeResponseResult,
} from "shared/compute/models/compute";
import { CellLineSelectorLines } from "shared/cellLineSelector/models/CellLineSelectorLines";
import {
  CellData,
  loadCellLines,
} from "shared/cellLineSelector/models/cellLines";
import {
  UploadFormat,
  UserUploadArgs,
  UploadTask,
...
import {
  ...
  formatAxisLabel,
  buildTraces,
} from "shared/interactive/utilities/interactiveUtils";
import { PageLinkAccordion } from "shared/interactive/components/PageLinkAccordion";
import Accordion from "shared/interactive/components/Accordion";
import {
  ControlledAssociationTable,
  ControlledAssociationTableProps,
} from "shared/interactive/components/AssociationTable";
import { StaticTable } from "shared/interactive/components/StaticTable";
import { VectorCatalog } from "shared/interactive/components//VectorCatalog";
import { Plot, PlotResizer } from "shared/interactive/components/Plot";
...
import "shared/interactive/styles/ControlledPlot.scss";
...
> {
  ...
  constructor(props: ControlledPlotProps, context: ApiContextInterface) {
    super(props, context);
  ...
  exportPoints = () => {
    if (
      ...
      this.state.traces[0].label.length == 0
    ) {
      alert("Plot has no points");
    } else {
      const params: Partial<ControlledPlotProps> = {
        features: [this.getSectionId("x"), this.getSectionId("y")],
        groupBy: this.getSectionId("color"),
        filter: this.getSectionId("filter"),
      };
      const url = `${
        window.location.origin + this.api.urlPrefix
      }/interactive/api/plot-points-csv?${encodeParams(
        params as {
          [key: string]: string;
        }
      )}`;
  ...
  updatePlotOnCustomAnalysisClick = (
    ...
    if (this.state.customAnalysisResult.type == "association") {
      ...
      y = sliceId;
    } else if (this.state.customAnalysisResult.type == "twoClass") {
      x = sliceId;
  ...
  render() {
    ...
    const customAnalysisModalProps = {
      showModal: this.state.showAssociationsModal || false,
      // "true" to account for blank strings.
      // this should never change, because these two props are from query params. this lets use just use initialDropdowns
      // in a previous implementation, one issue with prepopulating from changing state (vs the immutable URL query param) was that the modal was simply hidden/shown, so we need to reach in and call this.setStatesFromProps
      customAnalysisVectorDefault:
        this.props.defaultCustomAnalysisToX.toLowerCase() == "true"
          ? this.initialDropdowns.x
          : undefined,
      onHide: this.hideCustomAnalysisModal,
      queryInfo: {
        cellLineData: this.state.cellLineData,
        datasets: this.state.datasets || [],
        xDropdowns: this.vectorCatalogs.x
          ? this.vectorCatalogs.x.state.dropdowns
          : this.continuousRootDropdowns,
        yDropdowns: this.vectorCatalogs.y
          ? this.vectorCatalogs.y.state.dropdowns
          : this.continuousRootDropdowns,
      },
      onAssociationResultsComplete: this.onAssociationResultsComplete,
          ...
          <div className="section">
            <span className="button-wrapper">
              <ButtonGroup
                ...
                exportPoints={this.exportPoints}
