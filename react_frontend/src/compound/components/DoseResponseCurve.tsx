...
import { PlotResizer, Trace, DoseResponsePlot } from "./DoseResponsePlot";
...
export class DoseResponseCurve extends React.PureComponent<DoseResponseCurveProps> {
  ...
  buildPointsTraces(
    ...
    Array.from(groupedPointsByReplicate.keys()).forEach((replicate) => {
      const subgroup: Array<CurvePlotPoints> = groupedPointsByReplicate.get(
        replicate.toString()
      );
      ...
      isMaskedValues.forEach((isMaskedValue) => {
        const subsubGroup: Array<CurvePlotPoints> = groupedPointsByIsMasked.get(
  ...
  render() {
    const allKeys = new Set(
      this.props.curves.filter((curve) => !!curve.id).map((curve) => curve.id)
    );
