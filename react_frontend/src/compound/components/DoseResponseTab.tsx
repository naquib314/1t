...
import { LongTableData, ColumnType } from "shared/common/models/longTable";
...
type State = {
  ...
  doseResponseTable: Array<DoseResponseTableRow>;
    ...
    this.dapi
      ...
      .then((v) => {
        Promise.all([
          ...
          this.setState({
            curvePlotPoints: new Map(
              (values as Array<DoseResponseCurvePromise>).map(
                (v, i) =>
                  [
                  ...
                  ] as [string, DoseResponseCurvePromise]
              )
            ),
          });
        });
...
export default hot(DoseResponseTab);
