...
import { PlotlyDragmode } from "shared/plot/models/plotlyPlot";
import { Menu, Typeahead } from "react-bootstrap-typeahead";
import { ActiveSpan } from "../../trace";
...
function CelfiePage(props: CelfiePageProps) {
  // destructured props
  const {
    datasets,
    similarityOptions,
  ...
  const [highlightedGeneset, setHighlightedGeneset] = useState(null);
   ...
   */
  useEffect(() => {
    const datasetTaskIds: Array<string> = Object.values(datasetTasks);
    if (!datasetTaskIds.includes(null)) {
      Promise.all([
        ...datasetTaskIds.map((taskId) => getVolcanoData(taskId)),
  ...
  useEffect(() => {
    if (volcanoData.length > 0) {
      datasets.forEach((dataset, i) => {
        ...
        Plotly.newPlot(checkboxPlotsRef.current[i], plotlyTrace, plotlyLayout, {
          staticPlot: true,
        });
      });
    }
  ...
  */
  useEffect(() => {
    if (highlightedPoints && volcanoPlotRef.current) {
      ...
      const lineWidths = Array<number>(formattedData.length).fill(undefined);
  ...
  */
  useEffect(() => {
    ...
    if (highlightedGeneset) {
      ...
      } else if (highlightedGeneset.direction === "Neg") {
  ...
  */
  useEffect(() => {
    if (highlightedPoints && colorBy && networkPlotRef.current) {
      ...
      const lineWidths = Array<number>(nodes.length).fill(undefined);
      ...
      <Typeahead
        ...
        renderMenuItemChildren={(option, _, index) => (
          <div>
            ...
            {option.label}
            <div>
              <small>{`Task: ${selectedDatasetIds.find(
  ...
  const updateAnnotatedPoints = (selectedPoint: any) => {
    ...
    const pointToAdd: utils.TaskComputeResponseRow = formattedData.find(
...
CelfiePage.defaultProps = { startSpan: null };

export default hot(CelfiePage);
